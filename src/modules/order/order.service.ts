...
export async function createOrderService(
	...
	if (!mobil) throw new HTTPException(404, { message: "Mobil tidak ditemukan" });
	if (mobil.status !== "TERSEDIA")
		throw new HTTPException(400, { message: "Mobil sudah tidak tersedia" });
	if (mobil.order)
		throw new HTTPException(400, { message: "Mobil ini sudah memiliki pesanan" });
...
export async function uploadPembayaranService(
	...
	if (buktiBuffer && buktiBuffer.length > 0) {
		buktiTransferUrl = await uploadImageToCloudinary(buktiBuffer, "bukti-pembayaran");
	}
...
export async function updatePengambilanService(
	...
	if (suratJalanBuffer && suratJalanBuffer.length > 0) {
		suratJalanUrl = await uploadImageToCloudinary(suratJalanBuffer, "surat-jalan");
	}
